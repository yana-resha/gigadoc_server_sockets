...
const createOpenApiDocument = ({ host, port, webSocketPath }) => ({
  ...
  paths: {
    "/cycle": {
      get: {
        ...
        description:
          "Сохраняет прежний сценарий с avatar_state, имитацией речи и таймерами.",
        responses: {
  ...
  "x-websocket": {
    ...
    incoming: {
      avatar_state:
        "Существующий голосовой путь для /cycle и /decline-measurements.",
      mock_user_intent: {
    ...
    outgoing: {
      existing:
        "tech, scan_intro_ready, profile_questions_ready, scan_selection_ready, measurements_declined, session_exit_without_measurements, measurement_selected, measurement_started, measurement_results_ready, measurement_reset, results_intro_ready, params, qr",
      added: [
