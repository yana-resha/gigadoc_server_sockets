...
После старта сессии и каждого завершённого либо сброшенного замера stub также
передаёт `measurement_snapshot` с ревизией. В нём используются реальные зоны
SberMedAI: `fpg` и `cardio` для «Сердца и сосудов», `derm` для кожи и `vision`
для зрения. Это позволяет проверить независимые отметки «Бесконтактной оценки
сердца» и «Кардиокарты» в frontend без настоящего backend.
